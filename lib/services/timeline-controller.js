...
var Service = function(params) {
  ...
  var getSlugs = function(resourceName, pagetype) {
    pagetype = (['listpage', 'itempage'].indexOf(pagetype) >= 0) ? pagetype : 'itempage';
    return lodash.get(cfgResources, [resourceName, 'slug', pagetype], resourceName);
  };
  ...
  self.buildPageRouter = function() {
    ...
    router.route('/*').get(function(req, res, next) {
      var navigation = {};
      Promise.all([
        params.timelineHelper.getRandomPeriods(cfgResources['period']['menuSubItemCount'] || 4),
        params.timelineHelper.getRandomFacts(cfgResources['fact']['menuSubItemCount'] || 5),
        params.timelineHelper.getRandomFigures(cfgResources['figure']['menuSubItemCount'] || 10)
    ...
    router.route(util.format('/%s', getSlugs('period', 'listpage'))).get(function(req, res, next) {
      var count = 0, start = 0;
      var criteria = params.timelineHelper.criterionActiveDocs();
      Promise.resolve().then(function() {
        return params.timelineHelper.getPeriods(count, start, criteria);
    ...
    router.route([
      util.format('/%s', getSlugs('fact', 'listpage')),
      util.format('/%s/:pageNumber', getSlugs('fact', 'listpage'))
    ]).get(function(req, res, next) {
      var count, start, sorting = {};
      var criteria = params.timelineHelper.criterionActiveDocs();
      var pagination = {
        documentType: 'fact',
      ...
      if (pagination.enabled) {
        ...
        count = cfgResources['fact']['itemsPerPage'] || 10;
      ...
      Promise.resolve().then(function() {
        ...
        return params.timelineHelper.getFacts(count, start, criteria, sorting);
      }).then(function(facts) {
    ...
    router.route([
      util.format('/%s', getSlugs('figure', 'listpage')),
      util.format('/%s/:pageNumber', getSlugs('figure', 'listpage'))
    ]).get(function(req, res, next) {
      var count, start, sorting = {};
      var criteria = params.timelineHelper.criterionActiveDocs();
      var pagination = {
      ...
      if (pagination.enabled) {
        ...
        count = cfgResources['figure']['itemsPerPage'] || 10;
        start = count * (pagination.pageNumber - 1);
      }
      ...
      Promise.resolve().then(function() {
        ...
        return params.timelineHelper.getFigures(count, start, criteria, sorting);
    ...
    router.route(util.format('/%s', cfgResources['disclaimer'].slug)).get(function(req, res, next) {
      Promise.resolve().then(function() {
    ...
    router.route(util.format('/%s', cfgResources['aboutus'].slug)).get(function(req, res, next) {
      Promise.resolve().then(function() {
        return {};
      }).then(function(ctx) {
    ...
    router.route('/').get(function(req, res, next) {
      var latest = {};
      var criteria = params.timelineHelper.criterionActiveDocs();
      var sorting = { updatedAt: -1 };
      Promise.all([
        params.timelineHelper.getPeriods(cfgResources['period']['slideshowItemCount'] || 0, 0, criteria),
        params.timelineHelper.getFacts(cfgResources['fact']['slideshowItemCount'] || 8, 0, criteria, sorting),
        params.timelineHelper.getFigures(cfgResources['figure']['slideshowItemCount'] || 8, 0, criteria, sorting)
      ]).spread(function(periods, facts, figures) {
        var current = (periods.length > 0) ? lodash.random(periods.length - 1) : 0;
        res.render('pages/homepage', {
          contextPath: contextPath,
          resources: cfgResources,
          latest: {currentPeriod: current, periods: periods, facts: facts, figures: figures}
        });
      });
    });
    ...
    router.route('/events')
      .get(function(req, res, next) {
        ...
        if (!lodash.isEmpty(opts.filter)) {
          if (!lodash.isEmpty(opts.filter.q)) {
            criteria.headline = new RegExp(opts.filter.q, 'i');
          }
          if (lodash.isArray(opts.filter.id)) {
            var ids = lodash.filter(opts.filter.id, function(item) {
              return !(lodash.isEmpty(item) || item == "null");
            });
        ...
        params.timelineHelper.countEvents(criteria).then(function(total) {
          res.set('total-count', total);
          return params.timelineHelper.getEvents(count, start, criteria, sorting, options);
    ...
    router.route('/facts')
      .get(function(req, res, next) {
        ...
        params.timelineHelper.countFacts(criteria).then(function(total) {
          res.set('total-count', total);
          return params.timelineHelper.getFacts(count, start, criteria, sorting, options);
        }).then(function(records) {
          res.json(records);
        }, function(err) {
          res.send(err);
    ...
    router.route('/figures')
      .get(function(req, res, next) {
        ...
        params.timelineHelper.countFigures(criteria).then(function(total) {
          res.set('total-count', total);
          return params.timelineHelper.getFigures(count, start, criteria, sorting, options);
        }).then(function(records) {
          res.json(records);
        }, function(err) {
          res.send(err);
