...
var Service = function(params) {
  ...
  var infoModel = timelineMongooseWrapper.registerModel('InfoModel', {
    ...
    content: timelineMongooseWrapper.getSchema().Types.Mixed
  }, {
    collection: 'info'
  });
  ...
  var periodModel = timelineMongooseWrapper.registerModel('PeriodModel', {
    ...
    period: { type: String },
    picture: { type: String },
    state: { type: String, enum: ['active', 'pending', 'removed'] }
  }, {
    timestamps: true,
    collection: 'period'
  });
  ...
  var CRITERIA_ACTIVE_DOCUMENTS = { "$or": [{state: {"$exists": false}}, {state: 'active'}] };

  self.criterionActiveDocs = function() {
    return CRITERIA_ACTIVE_DOCUMENTS;
  };
  ...
  self.getInformation = function() {
    return Promise.resolve().then(function() {
      ...
      if (debuglog.isEnabled) {
        debuglog(' - other information content: %s', JSON.stringify(data, null, 2));
      }
      return data;
    });
  };
  ...
  self.getRandomPeriods = function(count) {
    ...
    var criteria = CRITERIA_ACTIVE_DOCUMENTS;
    return Promise.resolve().then(function() {
      return self.countPeriods(criteria);
    }).then(function(total) {
      return self.getPeriods(count, getStartPosition(count, total), criteria);
    });
  };
  ...
  self.getPeriodDetails = function(periodSlug) {
    ...
    return Promise.resolve().then(function() {
      ...
      var factQuery = factModel.find({ $and: [ {periodId: period._id}, CRITERIA_ACTIVE_DOCUMENTS ] });
      var figureQuery = figureModel.find({ $and: [ {periodId: period._id}, CRITERIA_ACTIVE_DOCUMENTS ] });
      return Promise.all([
  ...
  self.getEvents = function(count, start, criteria, sorting, options) {
    count = count || 0;
    start = start || 0;
    criteria = criteria || {};
    sorting = sorting || {};
    ...
    var query = eventModel.find(criteria).sort(sorting).skip(start).limit(count);
    return Promise.promisify(query.exec, {context: query})();
  };
  ...
  self.getRandomFacts = function(count) {
    ...
    var criteria = CRITERIA_ACTIVE_DOCUMENTS;
    return Promise.resolve().then(function() {
      return self.countFacts(criteria);
    }).then(function(total) {
      return self.getFacts(count, getStartPosition(count, total), criteria, {}, {
        keepOriginalData: false
      });
    });
  };

  self.getFacts = function(count, start, criteria, sorting, options) {
    count = count || 0;
    start = start || 0;
    criteria = criteria || {};
    sorting = sorting || {};
    ...
    var query = factModel.find(criteria).sort(sorting).skip(start).limit(count);
  ...
  self.getFactDetails = function(factSlug) {
    ...
    return Promise.resolve().then(function() {
      ...
      factDetails.events = lodash.filter(eventObjects, function(item) {
  ...
  self.getRandomFigures = function(count) {
    ...
    var criteria = CRITERIA_ACTIVE_DOCUMENTS;
    return Promise.resolve().then(function() {
      return self.countFigures(criteria);
    }).then(function(total) {
      return self.getFigures(count, getStartPosition(count, total), criteria, {}, {
        keepOriginalData: false
      });
    });
  };

  self.getFigures = function(count, start, criteria, sorting, options) {
    count = count || 0;
    start = start || 0;
    criteria = criteria || {};
    sorting = sorting || {};
    ...
    var query = figureModel.find(criteria).sort(sorting).skip(start).limit(count);
    if (options.keepOriginalData) {
      return Promise.promisify(query.exec, {context: query})();
    }
  ...
  self.getFigureDetails = function(figureSlug) {
    ...
    return Promise.resolve().then(function() {
      ...
      figureDetails.events = lodash.filter(eventObjects, function(item) {
        return !(item.state) || (item.state == 'active');
      });
