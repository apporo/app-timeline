...
var Service = function(params) {
  ...
  var connection = Connection({ mongoURI: cfgTimeline.mongodb.uri });
  var infoModel = InfoModel(connection);
  ...
  self.getRandomFacts = function(count) {
    ...
    return Promise.resolve().then(function() {
      ...
      var query = factModel.find({}).skip(getStartPosition(count, total)).limit(count);
      return Promise.promisify(query.exec, {context: query})();
    });
  };
  ...
  self.getFactTimeline = function(factSlug) {
    ...
    return self.getFactDetails(factSlug).then(function(factDetails) {
      timeline.title = {
        media: {
          url: '/filestore/picture/' + factDetails.picture + '/600/450/' + factDetails.slug + '.jpg'
        },
        text: {
  ...
  self.getRandomFigures = function(count) {
    ...
    return Promise.resolve().then(function() {
      ...
      var query = figureModel.find({}).skip(getStartPosition(count, total)).limit(count);
      return Promise.promisify(query.exec, {context: query})();
    });
  };
  ...
  self.getFigureTimeline = function(figureSlug) {
    ...
    return self.getFigureDetails(figureSlug).then(function(figureDetails) {
      timeline.title = {
        media: {
          url: '/filestore/picture/' + figureDetails.picture + '/600/450/' + figureDetails.slug + '.jpg'
        },
        text: {
  ...
  var transformEventsToSlides = function(events) {
    ...
    return lodash.map(events, function(event) {
      ...
      if (event.image) {
        slide.media = {};
        slide.media.url = '/filestore/picture/' + event.image + '/600/450/' + 'figure.jpg';
      }
